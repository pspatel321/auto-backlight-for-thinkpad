...
namespace Auto_Keyboard_Backlight.RawInput
{
    ...
    public class Device
    {
        ...
        public void RaiseInputReceived(object data)
        {
            InputReceived?.Invoke(this, new DeviceInputEventArgs(data));
        }
    }
    /// <summary>
    ...
    public class RawInput : FrameworkElement
    {
        ...
        {
            if (_hwnd == IntPtr.Zero) throw new Exception("Window not ready");
            if (cfg == null) cfg = GetRegisteredDeviceCollections();
            if (cfg.Count() == 0) return;
            var rids = new RAWINPUTDEVICE[cfg.Count()];
            int i = 0;
            foreach (DeviceCollection dc in cfg)
        ...
        /// </summary>
        /// <returns>Array of currently registered device collections</returns>
        public DeviceCollection[] GetRegisteredDeviceCollections()
        {
            return _registeredCollections.ToArray();
        }
        /// <summary>
